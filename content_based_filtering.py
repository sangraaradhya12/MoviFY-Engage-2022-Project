...
class content_based_filter:
    def __init__(self, links):
        ...
        #### READING MOVIES_FEATURES DATASET (SCRAPED FROM tMDB) ########
        ...
        ### LIMITING DATA TO TOP 600 MOVIES TO INCREASE SPEED ###############
        self.movies_features = self.movies_features.merge(
            self.movies_sorted["movieId"].head(600),
            left_index=True,
            right_on="movieId",
            how="right",
        )
        ...
        ########### MAKING INITIALIZE TF-IDF VECTORIZATION MODEL ##########################
    ...
    ######## COMPARING TWO PHRASES FOR SIMILARITY ##############################
    ...
    def update_similarities(self, user_given_summaries):
        self.movies_features["similarity"] = self.movies_features["features"].apply(
            lambda x: sum(
                [(self.get_similarity(x, summary))
                 for summary in user_given_summaries]
            )
    ...
    def recommend(self, features_to_include, user_given_summary):
        ...
        return pd.concat(
            [
                similarity_scores[similarity_scores["similarity"]
                                  > 0.01]["title"],
                self.links,
            ],
            axis=1,
